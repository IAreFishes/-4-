...
uint16_t TCS230_Period[4][VDATA_NUM+1]={0};  //存放RGB原始周期数据								  //更改

volatile uint16_t R_Val[4]={0};  //R值
volatile uint16_t G_Val[4]={0};  //G值
volatile uint16_t B_Val[4]={0};  //B值

uint32_t R_Pro[4][VDATA_NUM]={0};  //R白平衡比例值
uint32_t G_Pro[4][VDATA_NUM]={0};  //G白平衡比例值
uint32_t B_Pro[4][VDATA_NUM]={0};  //B白平衡比例值
...
{    
    char i = 0;  
    for(i=0;i<4;i++)	 //先读取flash里面的比例因子一次														  //更改
	{
...
/**
  ...
  * @retval None								
  */
uint16_t White_Line_Deviation(Discern_Type type)								//更改
{
    uint8_t i,max_site;
    uint16_t max = 0;
    u16 distance = 0;   //调整距离
    ...
    for(i=0,max=type.colour[0],max_site=0;i<3;i++)  //得到最大值位置 /*要得到最小值的位置,即检测到黑线的位置*/
	{
    ...
    LightBoard_RedLed(1<<max_site);
	if(ABS(type.colour[max_site]-type.min)<150)     //丢线	   //这里有问题，应该是 -type.max因为实际上max_site指向的是最小值位置，没有改名罢了
    {
        return distance = 90;    
    }
    else if((type.colour[max_site]-type.min)>(type.max-type.min)*0.25) //判断纠偏方向					 
    {																			 /*纠偏方向的判断有问题*/
        if(max_site<=1){adjust_direction = 1;}    /*改成4个之后需要进行更改*/
        else {adjust_direction = 2;}
    }
   
    K = (type.max-RefValue_min)/DWINDOW;       //刷新K值
    if(max_site==0)     //特殊处理
    {
        if((type.colour[max_site+1]-type.min)/K>=3)
        {
            distance = (3-max_site)*DWINDOW+((DWINDOW/2+5)-(type.colour[max_site+1]-type.min)/K);
            return distance;
        }
        else
        {
            distance = 70+(DWINDOW/2-7)+(DWINDOW-(type.colour[max_site]-type.min)/K);
            return distance;
        }
    }
    if(max_site==3)     //特殊处理
    {
        if((type.colour[max_site-1]-type.min)/K>=3)
        {
            distance = (max_site-4)*DWINDOW+((DWINDOW/2+5)-(type.colour[max_site-1]-type.min)/K);
            return distance;
        }
        else
        {
    ...
    if(max_site<=1)
    {
        if(type.colour[max_site+1]<type.colour[max_site-1])	   //这里的符号判断有问题 应该是<
        {
            distance = (1-max_site)*DWINDOW+((DWINDOW/2+5)-(type.colour[max_site+1]-type.min)/K);
            return distance;
        }
        else
        {
            distance = (1-max_site)*DWINDOW+10+((type.colour[max_site-1]-type.min)/K-(DWINDOW/2-5));
            return distance;
        }
    }
    else
    {
        if(type.colour[max_site+1]>type.colour[max_site-1])		//这里的符号判断有问题 应该是>				 
        {
            distance = (max_site-2)*DWINDOW+((DWINDOW/2+5)-(type.colour[max_site-1]-type.min)/K);
            return distance;
        }
        else
        {
            distance = (max_site-2)*DWINDOW+10+((type.colour[max_site+1]-type.min)/K-(DWINDOW/2-5));
            return distance;
        }
    }
}
...
uint16_t Judge_Colour(void)
{
    ...
    uint16_t Ref_RGB = 0;
    float RG_Ratio = 0,GB_Ratio = 0,RB_Ratio = 0;//颜色比率																			//更改	
    Discern_Type *Ref_Type;
    R_Type_Now = Scanning_Max_Min((uint16_t *)R_Val,4);Ref_R=R_Type_Now.max;  /*这里应该改成最大值，因为巡的是黑线。之前是最小值，是因为要找到除开白线以外的
																				深色场地颜色。现在如果还是最小值的话，那肯定是会找到黑线的_注释*/
    G_Type_Now = Scanning_Max_Min((uint16_t *)G_Val,4);Ref_G=G_Type_Now.max;
    B_Type_Now = Scanning_Max_Min((uint16_t *)B_Val,4);Ref_B=B_Type_Now.max;
