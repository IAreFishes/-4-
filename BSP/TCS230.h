...
extern uint16_t TCS230_Period[4][VDATA_NUM+1]; // 存放RGB原始数据								//更改
extern volatile uint16_t R_Val[4];  //R值
extern volatile uint16_t G_Val[4];  //G值
extern volatile uint16_t B_Val[4];  //B值

extern uint32_t R_Pro[4][VDATA_NUM];  //R白平衡比例值
extern uint32_t G_Pro[4][VDATA_NUM];  //G白平衡比例值
extern uint32_t B_Pro[4][VDATA_NUM];  //B白平衡比例值
